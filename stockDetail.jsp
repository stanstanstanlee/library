...
ISBN : <span>${data.bookNum}</span> <br><br>
도서제목 : <span>${data.bookTitle}</span> <br><br>
도서저자 : <span>${data.bookAuthor}</span> <br><br>

<form action="stockDetailSearch" method="post" id="searchForm"  onsubmit="searchStock(event);">
    <input type="hidden" name="bookNum" value="${data.bookNum}">
    <input type="hidden" name="bookTitle" value="${data.bookTitle}">
    <input type="hidden" name="bookAuthor" value="${data.bookAuthor}">
    ...
    <!--<button type="button" onclick="searchStock()">검색</button> <br><br>-->
</form>
...
<script>
   function cancelAction(bookNum) {
      location.href = 'bookDetail?bookNum=' + bookNum;
   }
